...
int Active = 0; //controlla quando si inserisce il seriale per la prima volta
...
void setup()   {                
  ...
  Serial.begin(250000);	// opens serial port, sets data rate to 256000 bps
  
  TCCR0B = TCCR0B & 0b11111000 | 0x2;
  TCCR1B = TCCR0B & 0b11111000 | 0x2;
  TCCR2B = TCCR0B & 0b11111000 | 0x2;
 
  int Active = S_WAIT_FOR_SF;
  State = S_WAIT_FOR_SF;
}
//---------------------------------------------------------------------------
...
void Pulse_PWM(){
  if(Active==S_WAIT_FOR_SF){
  for (int i=0;i<=255;i=i+10){
    analogWrite(r1_pin, i);
    analogWrite(g1_pin, i);
    analogWrite(b1_pin, i);
    delay(1000);
    }
